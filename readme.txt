
=== DTRT Content Sections ===
Contributors: dotherightthingnz
Donate link: http://dotherightthing.co.nz
Tags: html5, section, block, heading, split
Requires at least: 4.9.5
Tested up to: 4.9.5
Requires PHP: 5.6.30
Stable tag: 0.1.8
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Split the WordPress content area into sections.

== Description ==

Split the WordPress content area into sections.

== Installation ==

1. Upload the plugin files to the `/wp-content/plugins/wpdtrt-contentsections` directory, or install the plugin through the WordPress plugins screen directly.
2. Activate the plugin through the 'Plugins' screen in WordPress
3. Use the Settings->DTRT Content Sections screen to configure the plugin

== Screenshots ==

1. The caption for ./images/screenshot-1.(png|jpg|jpeg|gif)
2. The caption for ./images/screenshot-2.(png|jpg|jpeg|gif)

== Changelog ==

= 0.1.8 =
* Update wpdtrt-plugin-boilerplate to 1.4.39
* Sync with generator-wpdtrt-plugin-boilerplate 0.7.27

= 0.1.7 =
* Update wpdtrt-plugin-boilerplate to 1.4.38
* Sync with generator-wpdtrt-plugin-boilerplate 0.7.25

= 0.1.6 =
* Update wpdtrt-plugin-boilerplate to 1.4.36
* Sync with generator-wpdtrt-plugin-boilerplate 0.7.23

= 0.1.5 =
* Update wpdtrt-plugin-boilerplate to 1.4.25
* Sync with generator-wpdtrt-plugin-boilerplate 0.7.20

= 0.1.4 =
* Update wpdtrt-plugin-boilerplate to 1.4.24
* Prefer stable versions, but allow dev versions
* Fixes for PHPCS
* Add demo test

= 0.1.3 =
* Update wpdtrt-plugin-boilerplate to 1.4.22
* Rename wpdtrt-plugin to wpdtrt-plugin-boilerplate

= 0.1.2 =
* Fix package name
* Update wpdtrt-plugin to 1.4.21

= 0.1.1 =
* Update wpdtrt-plugin to 1.4.15

= 0.1.0 =
* Bump to the minimum version that can be updated by Composer

= 0.0.9 =
* Remove dev dependencies from composer.json including TGMPA

= 0.0.8 =
* Update wpdtrt-plugin to 1.4.14

= 0.0.7 =
* Fix path to autoloader when loaded as a test dependency
* Disable failing tests to enable package update for integration testing

= 0.0.6 =
* Include release number in wpdtrt-plugin namespaces
* Update wpdtrt-plugin to 1.4.6

= 0.0.5 =
* Update wpdtrt-plugin to 1.3.6

= 0.0.4 =
* Update wpdtrt-plugin to 1.3.1

= 0.0.3 =
* Migrate Bower & NPM to Yarn
* Update Node from 6.11.2 to 8.11.1
* Add messages required by shortcode demo
* Add SCSS partials for project-specific extends and variables
* Change tag badge to release badge
* Fix default .pot file
* Add tests
* Update wpdtrt-plugin to 1.2.30

= 0.0.2 =
* Bump version

= 0.0.1 =
* Initial version

== Upgrade Notice ==

= 0.0.1 =
* Initial release
